...
public class UserInfoDAO {
	...
	public int createUser(String familyName, String firstName, String familyNameKana, String firstNameKana, String sex,
			String email, String loginId, String password) {
	...
	// 強制ログアウト機能(session切れた時用)
	public int forcedLogout() {
		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();
		int result = 0;
		String sql = "update user_info set logined=0 where logined=1";
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			result = preparedStatement.executeUpdate();
		} catch (SQLException e) {
	...
	// ユーザー一覧
	public ArrayList<UserInfoDTO> userList() throws SQLException {
		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();
		ArrayList<UserInfoDTO> userInfoDTO = new ArrayList<UserInfoDTO>();
		String sql = "select * from user_info order by id ASC";
	...
	// ユーザー編集
	public void userUpdate(String loginId, String password, String familyName, String firstName, String familyNameKana,
			String firstNameKana, String sex, String email, String status, String id) throws SQLException {
		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();
		DateUtil DateUtil = new DateUtil();
		String sql = "update user_info set user_id=?, password=?, family_name=?, first_name=?, family_name_kana=?, first_name_kana=?, sex=?, email=?, status=?,update_date=? where id=?";
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, loginId);
			preparedStatement.setString(2, password);
	...
	// ユーザー登録(管理者)
	public int registUser(String familyName, String firstName, String familyNameKana, String firstNameKana, String sex,
			String email, String loginId, String password, String status) {
		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();
		int count = 0;
		String sql = "insert into user_info(user_id, password, family_name, first_name, family_name_kana,"
	...
	// 入力フォームでuser_id(unique)に同じ値が入ってないかチェック
	public int userIdCheck(String loginId) {
		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();
		int count = 2;
		String sql = "select count(user_id) as record_count from user_info where user_id=?";
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, loginId);
